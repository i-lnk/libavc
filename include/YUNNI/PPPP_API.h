...
//返回服务器可访问细节
typedef struct{
	struct sockaddr_storage svrAddr;//服务器地址
	int	 ifResGet;//是否有回应
}svrAccess_t,*pSvrAccess_t;
...
extern "C" {
  ...
  */
   ...
   */
    PPPP_API_API int PPPP_InitializeExt(
		char* LicenseString,
		unsigned char ifIpv6Pri,
		unsigned int MaxNmbOfSession,
		unsigned int MaxNmbOfChannel,
		unsigned int MaxSizeOfChannel,
		char *NodeName
		);
   ...
   */      
    PPPP_API_API int PPPP_NetworkDetect(
	 ...
	 PPPP_API_API int PPPP_NetworkDetectExt(
		 st_PPPP_NetInfo1* NetInfo,
		 unsigned short UDP_Port,
		 int *svrNmb,
		 svrAccess_t *svrAccess,
