...
CSearchDVS::CSearchDVS()
{
    ...
    pSearchDVS = this;
#else
	Yunni_SearchThreadRunning = 0;
	Yunni_searchThreadID = 0;
#endif
}
...
jumperr:
	...
	Yunni_SearchThreadRunning = 1;
	if(pthread_create(&Yunni_searchThreadID,NULL,CSearchDVS::PPPPSearchThread,this) < 0){
		Log3("YUNNI P2P Thread Create Fail! ");
		return 0;
	}
...
void CSearchDVS::Close()
{
	...
	if (Yunni_searchThreadID != 0) 
	{
        pthread_join(Yunni_searchThreadID, NULL);
        Yunni_searchThreadID = 0;
	}
}
...
//接收dvs发送来的信息
void* CSearchDVS::RecvThread(void * Param)
{
    CSearchDVS* pSearchDVS = (CSearchDVS*)Param ;
...
void CSearchDVS::YunniSearchProcess(){

	st_lanSearchExtRet devices[16];
	memset(devices,0,sizeof(st_lanSearchExtRet)*16);
...
//YunNI search 
void *CSearchDVS::PPPPSearchThread(void * param){
